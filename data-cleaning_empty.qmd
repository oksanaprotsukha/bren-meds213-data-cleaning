...
```{r}
#snowsurvey_csv %>% snowsurvey_csv$Snow_cover %>% unique
# snowsurvey_csv$Snow_cover %>% unique
...
species_long <- species_csv %>%
  pivot_longer(col = c('Alder_Flycatcher':'Black_headed_Gull'), names_to = 'species_type', values_to = 'count_observed')
```
...
#  mutate(species_presence = ifelse(species_count == "x", 0, species_count))
